...
def transcribe_audio_to_srt(audio_file_path: str, output_srt_path: str, language="zh", to_simplified=True):
    ...
    segments, info = model.transcribe(
        ...
        vad_parameters=dict(min_silence_duration_ms=500)  # 调整静音检测参数
    )
...
if __name__ == "__main__":
    audio_file = "简体.mp3"   # 替换为你的音频文件路径
    output_srt = "简体.srt"   # 替换为你要输出的字幕文件名称
    result_srt_path = transcribe_audio_to_srt(
        audio_file,
        output_srt,
        language="zh",
        to_simplified=True  # 设置为 True 输出简体中文，False 则保持原始输出
    )
